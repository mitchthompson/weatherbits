    ...
    $('button.forecast, form input[type=submit]').on('click', function(e) {
           ...
           if(formData[1] == ''){
            ...
            if (status == "success") {//if successful request
                ...
                for (i = 3; i <= 35; i += 8)  {    
                    $("#result").append('<div class="col-lg-2 col-md-5 col-xs-12  five-day text-center"><h4 class="date">' + formatDate(data["list"][i]["dt_txt"]) + "</h4>"
                    + '<div class="row"><div class="col-md-6 col-lg-12 col-xs-6"><img src="http://openweathermap.org/img/w/' + data["list"][i]["weather"][0]["icon"] + '.png"></div><!--col-md-6-->' 
           ...
           $(".feedback").html("Numbers are not a valid city name");
            ...
            if (status == "success") {//if successful request
                ...
                $("#result").html("<div class='city-name'><h3>" + data["name"] + " Current Weather</h3></div>");
                    ...
                    + '<div class="row"><div class="col-sm-6"><img src="http://openweathermap.org/img/w/' + data["weather"]["icon"] + '.png"></div><!--col-md-6-->'
                    + '<div class="col-sm-6"><ul>'    
                    + "<li>Temp: " + kelvinToF(data["main"]["temp"]) + " F</li>"
                    + "<li>Min Temp: " + kelvinToF(data["main"]["temp_min"]) + " F</li>"
                    + "<li>Max Temp: " + kelvinToF(data["main"]["temp_max"]) + " F</li>"
                    + "<li>Humidity: " + data["main"]["humidity"] + "</li>"
                    + '<li class="uppercase">Description: ' + data["weather"]["description"] + "</li></div><!--col-md-6--></div><!--row--></div><!--current-->" );
